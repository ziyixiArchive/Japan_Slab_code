...
def main(asdf_path,data_info_dir,stations_fname,output_dir):
    # load stations info
    stations=np.loadtxt(stations_fname,dtype=np.str)
    stations_mapper={}
    for row in stations:
        net_sta=f"{row[1]}.{row[0]}"
        stations_mapper[net_sta]=(float(row[2]),float(row[3]),float(row[4]))
    # load header info
    ...
    avaliable_net_sta=set(list(stations_mapper.keys())) & set(asdf_file.waveforms.list())
