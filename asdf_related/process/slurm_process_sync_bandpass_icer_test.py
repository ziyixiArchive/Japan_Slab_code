...
def get_scripts(run_files):
    ...
    for iiter in range(N_iters):
        ...
        for ieach in range(N_node):
            ...
            result += f"srun -N 1 -n {N_cores_each_node} --exclusive {PY} ../process/process_sync_bandpass.py --min_periods {min_periods} --max_periods {max_periods} --asdf_filename {filename} --waveform_length {waveform_length} --sampling_rate {sampling_rate} --output_directory {PROCESSED_DIR} --logfile {logfile} &"
        result += f"wait; "
...
def submit_job(thecommand):
    s = Slurm("process_sync", {"nodes": N_node, "ntasks": N_cores,
                               "time": "00:10:00", "cpus-per-task": 1, "mem-per-cpu": "2G"})
    s.run(thecommand)
