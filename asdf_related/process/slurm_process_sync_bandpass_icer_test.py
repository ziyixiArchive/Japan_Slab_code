...
def get_scripts(run_files):
    ...
    for iiter in range(N_iters):
        ...
        for ieach in range(N_node):
            ...
            result += f"srun -N 1 --ntasks={N_cores_each_node} ./process_sync_bandpass.py --min_periods {min_periods} --max_periods {max_periods} --asdf_filename {filename} --waveform_length {waveform_length} --sampling_rate {sampling_rate} --output_directory {PROCESSED_DIR} --logfile {logfile} &"
        result += f"wait; "
